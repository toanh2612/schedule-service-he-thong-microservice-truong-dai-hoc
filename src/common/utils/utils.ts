...
export const parseOrder = (order: any): OrderType => {
  return typeof order === "object" ? order : JSON.parse(order);
};

export const parseQuery = (
  query: QueryCommonDto<any> = {
    filter: {},
    filterOptions: {},
    order: {},
    page: null,
...
export abstract class AggregateRoot<T extends BaseEntity> {
  private _id: string;
  ...
  public apply(event: T): void {
    this._events.push(event);
    this._version++;
    this.on(event);
  }

  protected abstract on(event: T): void;

  public abstract commitEvents(): void;

  public loadFromHistory(history: T[]) {
    this._events = history;
  }
}
